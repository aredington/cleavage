...
(def ^{:private true} repository (memoize repository))
...
(defn- git-revisions
  ...
  (let [rw (RevWalk. repository)]
    ...
    (map #(.name %1) (seq rw))))
(def ^{:private true} git-revisions (memoize git-revisions))
...
(def ^{:private true} revision-sequences (memoize revision-sequences))
